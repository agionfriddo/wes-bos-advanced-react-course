...
export const permissions = {
  ...
  isAwesome({ session }: ListAccessArgs): boolean {
    return session?.data.name.includes('Andrew');
  },
};

// Rule based functions

// Rules can return a boolean - yes or no - or a filter which limits which products they can CRD.
export const rules = {
  ...
  canManageUsers({ session }: ListAccessArgs) {
    ...
    // 1. Do they have permission of canaManageUsers
